...
| 글로벌 **채용 물량은 -77%** | 월 934건(2021-11) → 216건(2026-08) | Hacker News |
| **신입 진입문은 한국이 오히려 넓다** | 국내 12.9% vs 해외 7.9% | 교차 분석 |
| 신입 문 축소는 **한국 전반의 현상이 아니다** | 공공 채용 신입 가능 **88.6%**, 6년간 유지 (직군은 다름) | 잡알리오 |
| 하락은 **직군별로 고르지 않다** | 프론트엔드 -36% vs 백엔드 -13% (시장 대비) | Hacker News |
| 국내도 **백엔드 > 프론트엔드** | 백엔드 606건 vs 프론트엔드 376건 (현재 게시분) | 국내 채용 플랫폼 |
...
| Hacker News (Algolia) | **불필요** | ✅ 채용글 39,362건 / 80개월 |
| Greenhouse 채용 보드 | **불필요** | ✅ 31개사 개발 JD 2,860건 |
| 국내 채용 플랫폼 공개 API | 불필요 | ✅ JD 373건 + 직군별 공고 2,529건 |
...
국내 직군별 공고 수(`data/domestic_role_*.csv`)는 별도 수집분이며, 수집 스크립트는
`domestic_jobs.json` 과 같은 이유로 저장소에 포함되어 있지 않습니다.
