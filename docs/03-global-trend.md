...
> **규모**: 80개월 / 채용글 **39,362건** (2026-08-11 재수집)
...
| :--- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 월평균 | 590 | **848** | 631 | 346 | 325 | 336 | 304 |
...
## 4. 🔬 신입 진입문 직접 비교 — 국내 12.9% vs 해외 7.9%
...
| **2026** | 2,434 | 1,236 | **7.9%** | **92.1%** |

![신입 진입문 비교](figures/entry_level_comparison.png)

* **해외 신입 수용 비중은 2022년 12.3%를 정점으로 2026년 7.9%까지 하락**했습니다.
* 명시된 최소 요구 연차의 **중앙값은 4년**(2,106건 기준).
* **국내 12.9% > 해외 7.9%** — 신입 진입문은 **한국이 오히려 넓습니다.**
* 전 기간 누적으로 보면 **1,839/18,787 = 9.8%** 입니다. 위 표는 연도별이라 자릿수가 다릅니다.
...
국내 공공 채용은 같은 기간 신입 가능 비중이 88~93%로 유지됐습니다
([02번 5장](02-domestic-analysis.md)). 다만 공공 데이터는 개발 직군만 분리할 수 없어,
"민간만 좁아졌다"로 곧장 연결하지는 않습니다.
...
**재현**: [src/analyze_role_demand.py](../src/analyze_role_demand.py) — HN 원본 재분석 (80개월 / 39,362건)
...
**직무명 기준**으로는 77%가 판정 불가입니다. 게임사는 직무명에 계층 대신 프로덕트명을
씁니다 — `Principal Software Engineer, Gameplay - Teamfight Tactics` 같은 식이라
대부분 `Software Engineer` 수준에서 멈춥니다.

그래서 **JD 본문의 엔진·기술 스택**으로 넓혀 다시 쟀습니다. Unity·Unreal·Godot 같은
엔진명이나 C++·셰이더·렌더링이 본문에 있으면 클라이언트 쪽으로 봅니다.

| 기준 | 클라이언트 | 서버 | 둘 다 | 판정 불가 |
| :--- | ---: | ---: | ---: | ---: |
| 직무명 | 58 | 26 | 0 | **282 (77%)** |
| JD 본문(엔진·스택) | 99 | 110 | 53 | **104 (28%)** |

엔진명만 따로 세면 366건 중 **118건(32.2%)** 이 본문에 언급합니다.

* 본문으로 넓히면 판정 불가가 **77% → 28%** 로 줄어 커버리지는 크게 좋아집니다.
* 대신 **정밀도가 떨어집니다.** 회사가 Unreal을 쓰면 QA·빌드·SDK 직무 본문에도 엔진명이
  적힙니다. "클라이언트 직무"가 아니라 **"클라이언트 스택을 다루는 공고"** 로 읽어야 합니다.
* 두 기준의 클라이언트:서버 비가 **2.2:1 과 0.9:1 로 서로 다릅니다.** 어느 쪽도 단독 인용 불가입니다.

### 그래도 추이는 못 봅니다

구성은 볼 수 있게 됐지만, 원래 질문("급격히 줄었나")은 여전히 답이 안 됩니다.

* 게임사 보드에는 마감 공고가 남지 않아 **단면뿐**입니다.
* HN에서 엔진명(Unity/Unreal/Godot)을 세면 연 **16~67건, 비중 0.4~0.9%** 입니다.
  추세를 논할 표본이 아닙니다.
...
5. **초판 수치에는 2020-03이 두 번 들어가 있었습니다.** 2020-03-23의 `Ask HN: Who is hiring right now?`는 코로나 시기 임시 스레드인데, 제목에 "who is hiring"이 들어가 정기 스레드로 함께 수집됐습니다. 그 달만 473건이 얹혀 채용글 총계가 39,831건으로 부풀었습니다. 제목에 `(March 2020)` 형식의 월-연도가 있는 스레드만 받도록 고치고 **2026-08-11에 전량 재수집**했습니다. 현재 문서의 모든 수치는 재수집분(80개월 / 39,362건) 기준입니다. 경위는 [04-lessons.md](04-lessons.md) 12번 참조.
