...
[03번 6장](03-global-trend.md)의 직군별 그래프가 해외 기준이었던 이유는 데이터가 없어서가
아니라 **수집 범위를 두 태그로 잡았기 때문**이었습니다. 채용 플랫폼의 직군 태그는 372개이고,
그중 개발 직군 20개를 다시 받았습니다.

![국내 직군별 공고](figures/domestic_role_counts.png)
...
고유 공고 2,529건. 한 공고에 직군 태그가 여러 개 붙어 **직군별 수를 더하면 안 됩니다**
(앱 단순합 154 → 중복 제거 114, 게임 클라이언트 21 → 16).

### 해외와 같은 방향, 다른 정도

두 수치는 **단위가 다릅니다.** 국내는 직군 태그가 붙은 공고 수(전수), 해외는 공고 본문에
그 직군이 언급된 비중(추정)입니다. 그래서 절대값 대신 **백엔드 대비 비(比)** 만 비교했습니다.
...
* **국내는 ML/AI가 백엔드를 넘어섰습니다**(1.06x). 해외는 아직 절반(0.48x)입니다.
* 인프라는 국내가 해외의 두 배 비중입니다. 해외에서 인프라가 크게 빠진 것과 대비됩니다.

### 공고 수 표기를 믿지 않은 이유

목록 페이지에는 직군마다 공고 수가 박혀 있어 요청 한 번이면 끝납니다. 그런데 목록 API를
끝까지 넘겨 직접 세어보니 값이 달랐고, **어긋나는 정도가 직군마다 달랐습니다.**

| 직군 | 페이지 표기 | API 실제 | 비율 |
| :--- | ---: | ---: | ---: |
| 머신러닝 엔지니어 | 498 | 642 | 129% |
| DevOps | 433 | 403 | 93% |
| 프론트엔드 | 529 | 376 | 71% |
| 서버 | 911 | 606 | 67% |
| 웹 개발자 | 721 | 152 | 21% |
...
비율이 18~129%로 흩어집니다. 일정했다면 보정이라도 하겠지만, 이대로 페이지 숫자를 쓰면
게임 직군만 5배 부풀려 읽게 됩니다. 그래서 **전부 API로 직접 셌습니다.**

> ⚠️ **국내도 단면입니다.** 과거 공고를 받을 수 없어 "프론트엔드가 줄었다"는
> 여전히 확인할 수 없습니다. 확인된 것은 **지금 시점의 구성**뿐입니다.
> 게임 클라이언트 16건은 이 플랫폼의 게임업계 커버리지가 낮은 탓일 수도 있어
> 국내 게임 시장의 크기로 읽으면 안 됩니다.
...
* **국내 직군별 분해는 단면까지만 됩니다.** 6장에서 20개 직군의 현재 공고 수를 받았지만, 과거 공고가 없어 **추이는 여전히 불가**합니다. 소스별 한계는 다음과 같습니다.

| 소스 | 직군 정보 | 왜 안 되나 |
| :--- | :--- | :--- |
| 민간 JD 373건 | 2개 카테고리뿐 | 수집 시 `AI/데이터`·`서버/백엔드` 태그만 골랐습니다 → **[6장](#6-직군별-공고-수--국내에도-있었다)에서 20개 직군으로 다시 받았습니다** |
