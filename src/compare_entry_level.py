...
def main() -> None:
    ...
    print("[해석 가능한 것]")
    if pub["비중"] > pri["비중"]:
        print(f"  · 같은 나라·같은 기간인데 공공 {pub['비중']:.1%} vs 민간 {pri['비중']:.1%}."
              f"\n    신입 문이 좁은 것은 한국 노동시장 전반의 성질이 아닙니다."
              f"\n    (다만 두 모집단의 직군이 같지 않습니다 — 아래 주의 참조)")
    ...
    # NCS '정보통신' 이 개발 직군을 뜻하지 않는다는 것을 수치로 보여준다.
    # 이 확인 없이 민간 개발 JD 와 나란히 놓으면 다른 것을 비교하게 된다.
    df = load_public()
    multi = df["ncsCdLst"].astype(str).str.contains(",")
    print(f"\n  · 결정적으로, 공공 모집단의 {multi.mean():.1%}({int(multi.sum()):,}건)는"
          " 정보통신 외 다른 NCS 대분류가"
          "\n    함께 붙어 있습니다 — 통합 공채에 태그가 하나 얹힌 것입니다."
          "\n    정보통신 단독 공고도 제목을 보면 '전산담당', '정보보안', '통신설비 유지보수'가"
          "\n    많아 **소프트웨어 개발 공고가 아닙니다.** NCS 대분류는 개발과 전산운영을 나누지 않습니다.")
    print("  ➔ 공공 수치는 '공공 채용 전반의 신입 개방도' 로만 읽으세요."
